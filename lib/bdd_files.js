...
function initialFileContent() {
    var result = '  def initialize\n';
    result += '    extend Desktop{0}Implements if $start_up_platform.platform.eql? "desktop"\n'.format(this.featureName);
    result += '    extend Android{0}Implements if $start_up_platform.platform.eql? "android"\n'.format(this.featureName);
    result += '    extend Ios{0}Implements if $start_up_platform.platform.eql? "iOS"\n'.format(this.featureName);
    result += '  end\n';
    return result;
}
